...
At normal widths, the persistent `BACKGROUND` panel and interactive `/workflow status` list/detail name materialized pending stages by display name and canonical ID. They show `<runId>:<stageId>` only when pre-start Intercom delivery is available and label unavailable delivery otherwise. Interactive status cards wrap a full target onto continuation rows when needed; the single-line panel metadata falls back to a clearly labeled canonical stage ID and an explicit `… N more` count rather than truncating an address or silently hiding additional stages. The narrow widget remains aggregate-only.
