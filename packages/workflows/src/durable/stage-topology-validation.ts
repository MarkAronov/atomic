...
export function immutableStageGroupError(stages: readonly DurableStageCheckpoint[]): string | undefined {
  ...
  for (const group of byReplayKey.values()) {
    ...
    if (topologies.some((topology) => topology.version !== DURABLE_STAGE_TOPOLOGY_VERSION
...
export function owningBoundaryIdError(
  ...
  const starts = stages.filter((stage) => stage.topology?.run?.runId === owningRunId
    && stage.topology.boundary?.event === "start");
...
export function sameRun(
  ...
  expected: DurableStageRunTopology,
): boolean {
  return actual !== undefined
    && actual.runId === expected.runId && actual.runName === expected.runName
...
export function promptOccurrenceIdentityError(
  stages: readonly DurableStageCheckpoint[],
): string | undefined {
  ...
  for (const legacy of stages) {
    ...
    const matches = stages.filter((current) => current.topology?.occurrenceKey !== undefined
...
function samePromptOccurrenceIdentity(
  legacy: DurableStageCheckpoint,
  current: DurableStageCheckpoint,
): boolean {
  ...
  return left !== undefined && right !== undefined
    ...
    && left.boundary === undefined && right.boundary === undefined;
}
