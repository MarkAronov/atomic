...
export function prepareDurableChildInvocation(input: DurableBoundaryDescriptor): DurableChildInvocation {
  ...
  const identity: DurableBoundaryRuntimeIdentity = {
    boundaryId,
    get childRunId() { return childRunId; },
    ...(topology !== undefined ? {
  ...
  return {
    ...
    async recordTerminal(status, output): Promise<void> {
      ...
      const parsedOutput = output === undefined
        ? undefined
        : parseWorkflowChildResult(output as WorkflowSerializableValue);
      if (status === "completed" && (parsedOutput === undefined
        || parsedOutput.runId !== childRunId || parsedOutput.workflow !== input.workflow)) {
      ...
      const terminalTopology = makeTopology(
        input, boundaryId, childRunId, topology.parentIds, topology.sourceOrder!, "terminal", status,
      );
      const priorTerminalAt = stages.filter((stage) => stage.replayKey === input.replayKey
...
export function validateDurableCompletedBoundary(input: DurableBoundaryDescriptor & {
  ...
  if (output.workflow !== input.workflow) throw invalid(input, "completed boundary workflow does not match the call site");
  ...
  if (boundary !== undefined) {
    ...
    if (resolved.lifecycle.latestTerminal?.checkpointId !== checkpoint.checkpointId
      || boundary.event !== "terminal" || boundary.status !== "completed"
      || boundary.child.runId !== output.runId || boundary.child.parentStageId !== checkpoint.topology?.stageId) {
  ...
  const ownership = reciprocalChildOwnership(
    ...
    input.workflowId, topology.stageId, input.rootWorkflowId,
  );
...
function validateStartRecord(
  ...
  if (topology === undefined || boundary === undefined || topology.sourceOrder === undefined
    || topology.version !== DURABLE_STAGE_TOPOLOGY_VERSION
    || topology.status !== "running" || boundary.status !== "running"
    || start.name !== input.boundaryName
    || boundary.version !== DURABLE_BOUNDARY_TOPOLOGY_VERSION
    || boundary.replayScope !== input.replayKey || boundary.alias !== input.alias
    || boundary.workflow !== input.workflow || boundary.child.runName !== input.childRunName
    ...
    || !sameRun(topology.run, input.runTopology)) throw invalid(input, "malformed or stale boundary-start ownership");
  ...
  if (boundary.invocationFingerprint === undefined) {
    const completedLegacy = stages.some((stage) => stage.replayKey === input.replayKey
      && stage.topology?.boundary?.event === "terminal"
  ...
  const duplicateOwner = stages.some((candidate) => candidate !== start
    && candidate.topology?.boundary?.event === "start"
...
function reciprocalChildOwnership(
  ...
  for (const group of byReplayKey.values()) {
    ...
    const claimsBoundary = runAware.some((stage) => {
      ...
      return run.runId === childRunId || run.parentRunId === parentRunId
        || run.parentStageId === parentStageId;
    });
    if (!claimsBoundary) continue;
    directGroups += 1;
    if (runAware.length === 0 || runAware.some((stage) => {
      const run = stage.topology!.run!;
      return run.runId !== childRunId || run.parentRunId !== parentRunId
        || run.parentStageId !== parentStageId || run.rootRunId !== rootRunId;
...
function validateScopedIdentities(
  ...
  for (const checkpoint of checkpoints) {
    const lookupIdentity = checkpoint.kind === "tool"
      ? checkpoint.argsHash
      : checkpoint.kind === "ui"
...
function validatedInvocationRecords(input: DurableBoundaryDescriptor): {
  ...
  const starts = stages.filter((checkpoint) =>
    checkpoint.replayKey === input.replayKey && checkpoint.topology?.boundary?.event === "start"
  );
