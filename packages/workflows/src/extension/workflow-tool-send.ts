...
function terminalWorkflowSendResultForRoot(
  rootRunId: string,
  stageId: string,
): WorkflowSendToolResult | undefined {
...
export async function workflowSendAction(
  ...
  const admitDelivery = (): void => {
    ...
    if (lease !== undefined && !lease.commit()) {
      throw new WorkflowSendAdmissionError(workflowSendResult(
        stageRunId,
        resolvedStageId,
        requestedDelivery,
        "noop",
  ...
  const awaitAdmission = <T>(operation: Promise<T>): Promise<T> =>
    Promise.race([operation, terminalPending.promise]);
  try {
