...
- Added a `github-copilot/gemini-3.8-flash` entry for the model GitHub shipped in Copilot on 2026-09-03. models.dev publishes no `github-copilot` entry for it, so the generator supplements one from Copilot's own `gemini-3.7-flash` entry and corrects every field the authenticated Copilot `/models` endpoint publishes: a 1,048,576-token context window, 65,536 maximum output tokens, and `reasoning_effort` of `low`, `medium`, and `high`. Its `supported_endpoints` is `["/chat/completions"]`, so it routes through Copilot's OpenAI-compatible completions endpoint with Copilot's static headers rather than through Google's API, and it now sends `reasoning_effort` with a level map limited to those three efforts. Only cost — whose Copilot `token_prices` match — and modalities remain inherited from the sibling. The supplement and its corrections are skipped as soon as models.dev publishes the entry.
