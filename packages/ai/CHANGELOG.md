...
- Documented that generated `input` metadata lists only the modalities Atomic can send. All 14 Anthropic models in the published catalog advertise a `pdf` input modality, Claude Fable 5.1 among them, but `Model.input` has no `"pdf"` member and there is no document content block, so every Claude entry narrows to `["text", "image"]` as it always has. PDF input remains unsupported across the catalog.
