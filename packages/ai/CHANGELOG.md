...
- Documented that generated `input` metadata lists only the modalities Atomic can send. Several providers publish a `pdf` input modality for Claude models, Claude Fable 5.1 included, but `Model.input` has no `"pdf"` member and there is no document content block, so Claude entries narrow to `["text", "image"]` as they always have.
