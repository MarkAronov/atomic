...
- Fixed forced tool use being sent to Claude Fable 5.1, which rejects `tool_choice: {"type": "any"}` and `{"type": "tool", ...}` on every request with a 400, whichever platform serves it. The Anthropic and Bedrock entry points accept those shapes, so a caller could construct a request the model was guaranteed to refuse. A model marked `supportsForcedToolChoice: false` now **rejects the request with an error** naming the model and Anthropic's documented remedy, rather than sending it. An earlier revision of this branch silently substituted `{"type": "auto"}`; that discarded an explicit caller instruction and is no longer done. `auto` and `none` are never altered, and every other model passes forced choices through unchanged. The flag is applied to every mirror of the model, not only to `provider: "anthropic"`, because the rejection is a property of the model rather than of Anthropic's endpoint.
