...
- Added a `github-copilot/gemini-3.8-flash` entry for the model GitHub shipped in Copilot on 2026-09-03. When models.dev omits the entry, the generator supplements one from Copilot's own `gemini-3.7-flash` entry; when models.dev lists it with generic or stale values, the generator preserves upstream-owned metadata while correcting every field the authenticated Copilot `/models` endpoint publishes: a 1,048,576-token context window, 65,536 maximum output tokens, and `reasoning_effort` of `low`, `medium`, and `high`. Its `supported_endpoints` is `["/chat/completions"]`, so it routes through Copilot's OpenAI-compatible completions endpoint with Copilot's static headers rather than through Google's API, and it sends `reasoning_effort` with a level map limited to those three efforts. Only cost — whose Copilot `token_prices` match — and modalities remain inherited from the sibling when supplementation is required.
