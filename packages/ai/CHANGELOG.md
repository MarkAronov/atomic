...
- Added Gemini 3.8 Flash to the generated provider catalogs. `google` and `google-vertex` publish `gemini-3.8-flash` with a 1,048,576-token context window, 65,536 maximum output tokens, text and image input, $0.75 input / $3.75 output / $0.075 cache read per million tokens, and always-on thinking (`off` is denied, as on Gemini 3.6 and 3.7 Flash). The model also lands on `opencode`, `openrouter` (`google/gemini-3.8-flash` and `:batch`), and `vercel-ai-gateway` (`google/gemini-3.8-flash`) from those providers' own catalogs ([#9076](https://github.com/earendil-works/pi/issues/9076)).
- Added a `github-copilot/gemini-3.8-flash` entry for the model GitHub shipped in Copilot on 2026-09-03. It routes through Copilot's OpenAI-compatible completions endpoint with Copilot's static headers and `supportsReasoningEffort: false`, not through Google's API, so no thinking level map is generated. models.dev publishes no Copilot entry for this model and GitHub publishes no limits or pricing for it, so the entry's 1,000,000-token context window, 64,000 maximum output tokens, cost, and modalities are **inherited from Copilot's own `gemini-3.7-flash` entry** and the API model id follows Copilot's plain-Google-id convention. The supplement is skipped as soon as models.dev publishes the entry.
