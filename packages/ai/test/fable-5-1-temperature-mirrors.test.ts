...
describe("Bedrock temperature suppression for Claude Fable 5.1", () => {
	...
	] as const)("keeps inferenceConfig.temperature for %s", async (modelId) => {
		const model = getModel("amazon-bedrock", modelId);
...
describe("OpenAI-completions temperature suppression for Claude Fable 5.1", () => {
	...
	it.each(["anthropic/claude-fable-5", "anthropic/claude-opus-5"] as const)(
...
describe("temperature suppression stays narrowly scoped", () => {
	it("marks exactly the Claude Fable 5.1 Bedrock and OpenRouter entries", () => {
		// The broader Anthropic temperature predicate matches many more models on these two APIs.
		// Only Claude Fable 5.1 is opted in here; the rest keep their pre-existing behavior.
		const bedrock = getModel("amazon-bedrock", "anthropic.claude-fable-5-1");
		const openrouter = getModel("openrouter", "anthropic/claude-fable-5.1");

		expect(bedrock.compat?.supportsTemperature).toBe(false);
		expect(openrouter.compat?.supportsTemperature).toBe(false);
		expect(getModel("amazon-bedrock", "anthropic.claude-fable-5").compat?.supportsTemperature).toBeUndefined();
		expect(getModel("amazon-bedrock", "eu.anthropic.claude-fable-5").compat?.supportsTemperature).toBeUndefined();
		expect(getModel("openrouter", "anthropic/claude-fable-5").compat?.supportsTemperature).toBeUndefined();
	});
