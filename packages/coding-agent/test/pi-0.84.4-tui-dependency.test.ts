...
function fakeFd(base: string): string {
	const path = join(base, "fd-fixture.mjs");
	writeFileSync(
		path,
		`#!/usr/bin/env node
...
describe("pi-tui 0.84.4 dependency behavior", () => {
	test("scoped autocomplete keeps a direct child ahead of flooded nested matches", async () => {
		const base = tempDir();
		makeDirectory(join(base, "scope/projects"));
		for (let index = 1; index <= 250; index += 1) {
			makeDirectory(
				join(base, `scope/a${String(index).padStart(3, "0")}/venv/lib/python3.12/site-packages/pkg/core/profile`),
			);
		}
