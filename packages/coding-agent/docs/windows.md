...
The same source path is used for package-manager and release-archive builds. Node installs retain Node's persistent compile cache and its `NODE_DISABLE_COMPILE_CACHE=1` coverage opt-out. Release builds syntax-minify the shared application sidecar without shortening identifiers. Windows x64 and ARM64 keep launcher bytecode disabled solely pending Atomic-specific target-machine smoke and stress evidence on both architectures. A pinned Bun 1.4.0 probe now cross-compiles bytecode launchers for `bun-windows-x64-baseline` and `bun-windows-arm64` and verifies their PE machine types, but cross-compilation does not execute either launcher. Atomic observed a Bun 1.3.14 Windows `llint_entry` startup crash but has not reproduced it on pinned Bun 1.4.0. Bun 1.4.0 includes the embedded-bytecode alignment fix ([#26299](https://github.com/oven-sh/bun/pull/26299)) and bytecode integrity fallback ([#31961](https://github.com/oven-sh/bun/pull/31961)); a separate Bun 1.4.0 Windows standalone/JIT report ([#40302](https://github.com/oven-sh/bun/issues/40302)) is not demonstrated to be bytecode-specific. The fallback does not mean Bun 1.4.0 retains the alignment bug. Enabling bytecode still requires full-archive x64 and ARM64 Windows runs that cover the TUI, workflows, tools, extensions, workers, and native add-ons under smoke and stress load. The current unreleased Windows startup candidate has not completed those runs or the installed-Node smoke, so these notes do not claim measured speedup or target compatibility evidence.
## Filesystem Watchers
