...
The same source path is used for package-manager and release-archive builds. Node installs retain Node's persistent compile cache and its `NODE_DISABLE_COMPILE_CACHE=1` coverage opt-out. Release builds syntax-minify the shared application sidecar without shortening identifiers. Windows x64 and ARM64 launchers compile without bytecode: despite the Bun 1.4.0 embedded-bytecode alignment fix ([#26299](https://github.com/oven-sh/bun/pull/26299)) and integrity fallback ([#31961](https://github.com/oven-sh/bun/pull/31961)), Windows standalone executables cross-compiled with bytecode from a non-Windows host segfault before user code runs, even on `--version` — the release payload is built on Linux, so shipped Windows launchers crashed while the Windows-hosted smoke build passed. Linux and macOS launchers keep embedded bytecode. The pinned Bun 1.4.0 probe cross-compiles bytecode `bun-windows-x64-baseline` and `bun-windows-arm64` launchers and verifies their PE machine types, so re-enabling can be re-evaluated against future Bun releases. Cross-compilation does not prove runtime compatibility, so release validation must still exercise the full archive, TUI, workflows, tools, extensions, workers, and native add-ons on Windows x64 and real Windows ARM64 hardware.
## Filesystem Watchers
