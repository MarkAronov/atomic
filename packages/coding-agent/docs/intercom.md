...
A recoverable disconnect is only reported where someone is waiting on it. Work Intercom starts on its own — eager workflow-stage warm-up during `session_start` and the background subagent/pending-stage event relays — stays silent when the broker socket drops, because the failed lazy attempt is discarded and the next call reconnects; the stage keeps running rather than showing an extension error. Explicit `intercom` calls, `/intercom`, and the ALT+M overlay still fail visibly, and protocol, authentication, configuration, non-recoverable initialization, and terminal relay failures are reported on every path. Classification is by error type raised inside the broker client, so an identically worded failure from anywhere else stays actionable.
