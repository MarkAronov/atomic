...
- **Stable durable graph**: stage, task, chain, parallel, and child checkpoints preserve source stage ids/order, parent DAG edges, actual status, owning run/boundary metadata, timing, model, output, retained chat-session references, and exact `{ runId, stageId }` targets. Fresh-process resume and completed inspection reconstruct the same expanded graph directly from DBOS.
...
Selecting a completed target follows a separate read-only open path. Atomic reconstructs root and reciprocal nested child-run snapshots from authoritative checkpoints with their durable source ids, source order, parent edges, status, run ownership, and exact control targets. It never calls the resume dispatcher or runs workflow code, tools, tasks, or prompts. Completed graphs remain inspectable even when no retained chat transcript survives.
