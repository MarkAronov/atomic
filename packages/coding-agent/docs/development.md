...
Compiled releases syntax-minify the shared CJS `app.js` sidecar for all eight supported targets without identifier minification. Linux and macOS keep bytecode. Windows x64 and ARM64 keep source payloads solely until Atomic completes target-machine smoke and stress coverage on both architectures. `bun run scripts/probe-windows-bytecode.ts` pins Bun 1.4.0, cross-compiles bytecode launchers for `bun-windows-x64-baseline` and `bun-windows-arm64`, and verifies their PE machine types. This checks the candidate build path only; it does not execute a Windows binary. Atomic observed a Bun 1.3.14 Windows startup crash in `llint_entry` but has not reproduced it on the pinned Bun 1.4.0 runtime. Bun 1.4.0 already contains the embedded-bytecode alignment fix ([#26299](https://github.com/oven-sh/bun/pull/26299)) and integrity fallback ([#31961](https://github.com/oven-sh/bun/pull/31961)); the remaining Bun 1.4.0 Windows report ([#40302](https://github.com/oven-sh/bun/issues/40302)) concerns a general standalone/JIT segfault and has not been shown to be bytecode-specific. Enabling Windows bytecode still requires full-archive x64 and ARM64 target-machine runs that cover the TUI, workflows, tools, extensions, workers, and native add-ons under smoke and stress load. The fallback does not imply that Bun 1.4.0 retains the alignment defect.
