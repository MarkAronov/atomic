...
- `input` describes the modalities **Atomic can send**, not everything the upstream model accepts. `["text"]` and `["text", "image"]` are the only values, and every Claude entry uses `["text", "image"]`. That matches what Anthropic publishes for these models: the [Claude Fable 5.1 overview](https://platform.claude.com/docs/en/models/fable-5-1/overview) capability table lists `Input → output: Text and images → text`, and the page never mentions PDF. Anthropic documents PDF separately as a *platform* feature — ["All active models support PDF processing"](https://platform.claude.com/docs/en/build-with-claude/pdf-support) — handled through the same vision path as images rather than as a per-model modality, which is why all 14 Anthropic models in the models.dev catalog carry an identical `pdf` flag. Atomic has no document content block and no code path that produces one, so **PDF input is not supported for any model today**; extract the text first, for example with the `read` tool. Because the capability is uniform across the family, this is not something a single model's metadata can express.
...
`supportsTemperature` also exists on the Amazon Bedrock and OpenAI-compatible completions `compat` objects, with the same meaning and the same `true` default: when `false`, the provider omits `temperature` from the request. Atomic sets it `false` on the Claude Fable 5.1 entries for those APIs, because that model rejects non-default `temperature`, `top_p`, and `top_k` on every request. The flag is applied per model rather than per family, so every other model on those providers keeps sending `temperature` exactly as before.
