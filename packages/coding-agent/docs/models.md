...
Atomic's own agent loop only ever asks for `auto` or `none`, so no interactive session can reach this. It is reachable through the `@bastani/pi-ai` library's Anthropic and Bedrock entry points, which accept the wider tool-choice shape. For a model marked `supportsForcedToolChoice: false`, those entry points **fail the request with an error naming the model and the remedy**, before the round trip.
