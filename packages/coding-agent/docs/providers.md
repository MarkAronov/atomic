...
Chat requests authenticated with a raw `COPILOT_GITHUB_TOKEN` (including `github_pat_`, `ghp_`, `gho_`, and `ghu_` tokens) send `Copilot-Integration-Id: copilot-developer-cli`. A `Copilot-Integration-Id` supplied by `models.json` or per request takes precedence. Exchanged OAuth tokens containing a `tid=` segment keep the existing OAuth headers unchanged. The env-token routing implementation and its exported helpers now live in `@bastani/pi-ai`. A `models.json` value set exactly to `vscode-chat` is treated as the built-in default and replaced for raw-token requests; use a per-request `Copilot-Integration-Id` header to force any value.
