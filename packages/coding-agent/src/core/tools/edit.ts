...
export const editToolSystemPromptContribution = Object.freeze({
	...
	guidelines: Object.freeze([
		...
		[
			"Anti-patterns:",
			"```text",
			"# WRONG — `-` rows / bare context rows are invalid: `-` rows are not valid; hashline ranges already name the lines being changed.",
			"replace 3..3:",
			'    msg = "Hello"',
			"-   print(msg)",
			"+   return msg",
			...
			"# RIGHT: delete 2..3",
			"",
			"# WRONG — empty `insert` / `replace`; `insert` needs at least one `+TEXT` body row (and so does `replace`).",
			"insert after 2:",
			"replace 4..4:",
			"# To delete line 4, RIGHT: delete 4",
			"",
