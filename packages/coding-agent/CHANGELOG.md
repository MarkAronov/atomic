...
- The models documentation now states that a model's `input` field lists only the modalities Atomic can send. Providers publish a `pdf` input modality for Claude models including Fable 5.1, and those models do accept PDFs through the Anthropic API, but Atomic has no document content block, so PDF input is not available and Claude entries stay at `["text", "image"]`.
