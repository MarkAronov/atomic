...
- The models documentation now states that a model's `input` field lists only the modalities Atomic can send. All 14 Anthropic models in the published catalog advertise a `pdf` input modality, Fable 5.1 among them, and those models do accept PDFs through the Anthropic API, but Atomic has no document content block, so PDF input is unavailable for every model and Claude entries stay at `["text", "image"]`. Extract the text first.
