...
- Fixed forced tool use reaching Claude Fable 5.1, which rejects it on every request whichever platform serves it. Atomic's agent loop only ever asks for `auto` or `none`, so no interactive session was affected, but the bundled `@bastani/pi-ai` library's Anthropic, Amazon Bedrock, and OpenAI-completions entry points all accept the wider tool-choice shape. Such a request now fails with an error naming the model and the remedy, rather than being sent or silently rewritten to `auto`. Through a gateway the old behavior was quieter and worse: OpenRouter drops parameters a model does not support, so the forced choice disappeared and the answer looked normal. The guard matches the versioned Claude Fable 5.1 model id, so it covers whichever mirrors a provider currently publishes. Claude Fable 5 accepts forced tool use and is untouched, and an unversioned "latest" alias is not covered because its target cannot be known from its id.
