...
- The models documentation now states that a model's `input` field lists only the modalities Atomic can send, and cites why `["text", "image"]` is correct for Claude rather than a narrowing: Anthropic's own capability row for Claude Fable 5.1 reads `Input → output: Text and images → text`, and Anthropic documents PDF separately as a platform feature ("All active models support PDF processing") routed through the same vision path as images. That is why all 14 Anthropic catalog entries carry an identical `pdf` flag. Atomic has no document content block, so PDF input is unavailable for every model; extract the text first.
...
- Fixed forced tool use reaching Claude Fable 5.1, which rejects it on every request whichever platform serves it. Atomic's agent loop only ever asks for `auto` or `none`, so no interactive session was affected, but the bundled `@bastani/pi-ai` library's Anthropic, Amazon Bedrock, and OpenAI-completions entry points all accept the wider tool-choice shape. Such a request now fails with an error naming the model and the remedy, rather than being sent or silently rewritten to `auto`. Through a gateway the old behavior was quieter and worse: OpenRouter drops parameters a model does not support, so the forced choice disappeared and the answer looked normal. The guard covers the seven versioned Claude Fable 5.1 mirrors; Claude Fable 5 accepts forced tool use and is untouched, and an unversioned "latest" alias is not covered because its target cannot be known from its id.
...
- Fixed a server-side fallback losing track of which model answered. When Claude Fable 5.1 declines partway through a response and Anthropic retries on Claude Opus 4.8 or Opus 5 within the same stream, the handoff marker was discarded, the reply stayed labelled as Fable 5.1, and the turn was costed at Fable 5.1's rates rather than at those of the model that produced it. Atomic now records the marker, reports the serving model, and prices the turn correctly. Continuing that conversation also follows Anthropic's replay rules, so a follow-up turn is not rejected for echoing reasoning from both sides of the handoff.
