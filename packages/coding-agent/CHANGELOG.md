...
- Gemini 3.8 Flash is now selectable in Atomic on Google (`google/gemini-3.8-flash`), Google Vertex, GitHub Copilot, opencode zen, OpenRouter (`openrouter/google/gemini-3.8-flash` and its `:batch` variant), and the Vercel AI Gateway. On the Google APIs it carries a 1,048,576-token context window, 65,536 maximum output tokens, and always-on thinking, so `off` is unavailable and `low`, `medium`, and `high` behave as they do on Gemini 3.7 Flash. On GitHub Copilot it runs through Copilot's own OpenAI-compatible endpoint with Copilot's platform limits, and exposes no selectable reasoning effort because that endpoint accepts none. Run `--list-models` for the models your account actually has access to; Copilot availability follows GitHub's rollout and administrator policy ([#9076](https://github.com/earendil-works/pi/issues/9076)).
