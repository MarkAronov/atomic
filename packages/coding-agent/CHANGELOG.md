...
- Fixed forced tool use reaching Claude Fable 5.1, which rejects it with a 400 on every request whichever platform serves it. Atomic's agent loop only ever asks for `auto` or `none`, so no interactive session was affected, but the bundled `@bastani/pi-ai` library's Anthropic and Bedrock entry points accept the wider tool-choice shape. Such a request now fails with an error naming the model and the remedy, rather than being sent or silently rewritten to `auto`. The guard covers every mirror of the model, not just the Anthropic one.
