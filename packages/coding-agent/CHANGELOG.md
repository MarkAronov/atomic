...
- Added the optional `fail(reason: Error)` member to the `WorkflowPendingStageDelivery` extension-context interface. A delivery owner that has run out of recovery uses it to settle the stage's pending delivery terminally, so a stage waiting on `ready()` for messages that can no longer be drained fails at its own lifecycle boundary instead of waiting indefinitely. Existing structural implementers are unaffected; a delivery that omits it keeps the previous wait-for-delivery behavior.
