...
- Claude Fable 5.1 (`claude-fable-5-1`) is now selectable in Atomic wherever a configured provider publishes it — currently Anthropic, Amazon Bedrock, opencode zen, OpenRouter, and the Vercel AI Gateway, with a provider "latest" alias such as OpenRouter's `~anthropic/claude-fable-latest` also routing to it. It offers a 1M-token context window, 128K maximum output, always-on adaptive thinking at `low`, `medium`, `high`, `xhigh`, or `max`, and a $0.25 per million cache read against Claude Fable 5's $1.00. Run `--list-models` for the current set; provider catalogs change independently of Atomic releases.
...
- Fixed forced tool use reaching Claude Fable 5.1, which rejects it with a 400 on every request. Atomic's agent loop only ever asks for `auto` or `none`, so no interactive session was affected, but the bundled `@bastani/pi-ai` library's Anthropic entry point accepts the wider tool-choice shape. A forced choice is now downgraded to `auto` for that model, matching Anthropic's documented guidance; every other model is unchanged.
- Fixed Claude Fable 5.1 sending `temperature` when used through Amazon Bedrock or OpenRouter. The Anthropic path already omitted it, but the Bedrock Converse and OpenAI-completions request builders emitted the field for every model, so the three Bedrock inference profiles and the OpenRouter entry sent a parameter Claude Fable 5.1 rejects. Those four entries now omit it. Every other model on those providers is unchanged.
