...
A stage that never receives its queued messages fails rather than waiting forever. If Intercom cannot reconnect during that stage's bounded warm-up, the delivery is settled with a terminal, stage-scoped error and the stage ends `failed` — its queued entries stay queued for a later run rather than being recorded as delivered, and no extension diagnostic is written into the root session's transcript. A stage with nothing queued is unaffected and starts normally.
