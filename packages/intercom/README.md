...
A workflow stage warming up before its heavy module exists is the one case the reconnect backoff cannot own, so the lightweight wrapper retries that warm-up on the same bounded schedule. When those attempts run out it writes nothing to the console: it hands the stage's pending delivery a typed terminal reason, and the workflow side fails that stage with a stage-scoped error instead of leaving it waiting on `pendingStageDelivery.ready()`. Queued messages are left queued rather than delivered to a stage that will not read them.
