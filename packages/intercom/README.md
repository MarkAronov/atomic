...
Live target lookup accepts only an exact full Intercom session ID or an exact case-insensitive session name. `intercom list` also returns materialized workflow-stage rows labeled `PENDING` or `RUNNING`, with canonical `<runId>:<stageId>` targets and actual groups. The invocation context `workflow:<rootRunId>` has directional list/send/live-ask control over stages in owned `workflow:<rootRunId>/...` subgroups. Subgroup siblings remain mutually isolated, other root runs and unrelated sessions cannot cross in, and explicit `group: "default"` remains non-owned.

**`send`** — Sends through ordinary Intercom. Live workflow-stage sessions receive messages immediately. Known pending invocation-owned stages queue durably and return `queued`; only the owning invocation group can use that route. Pending `group: "default"` stages remain ineligible. Queues retain the 50-message cap, persistence, deduplication, and lifecycle failure behavior.

**`ask`** — Sends a message and waits for a live recipient to reply (10-minute timeout). Invocation control supports a live ask into an owned isolated subgroup, and the exact broker-recorded reply crosses back to that asker without opening reverse or lateral group access. Ask to an uninitialized stage remains refused with `pending_stage_ask_unsupported`; use queued `send` instead.
