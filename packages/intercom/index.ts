...
For send, live session names and exact full session IDs remain supported. For a known workflow stage, use the exact \`workflow:<rootRunId>/<segment>[/<segment>...]\` path; send messages to pending stages queue automatically. Pattern paths containing \`*\` or \`**\` are recognized but not yet accepted.
Usage:
...
"default" is the shared group; "true" and "auto" are reserved for subagent auto-groups. Joining does not grant cross-group access; contact_supervisor remains the only cross-group path.`,
		...
		parameters: Type.Object({
			...
			to: Type.Optional(Type.String({ description: "Live session name, exact full session ID, or exact `workflow:<rootRunId>/<segment>[/<segment>...]` for a known workflow stage; pattern paths are not yet accepted; sends to pending stages queue automatically (for 'send', 'ask', or targeted 'reply')" })),
