...
import { TRUNCATED_MARKER, truncateToBytes } from "../../packages/coding-agent/src/core/model-routing-bytes.js";
...
for (const unit of ["a", "界", "😀", '"\\\n\t\u0000']) {
	test(`routing excerpts bound JSON bytes without splitting Unicode: ${JSON.stringify(unit)}`, () => {
		const task = `START ${unit.repeat(120_000)} END`;
...
test("multiple, nested, mixed-case and unclosed protected spans survive in source order", () => {
	...
	const task = `${"a".repeat(60_000)}${first}${"b".repeat(60_000)}${second}${"c".repeat(60_000)}${unclosed}`;
	const excerpt = modelRoutingTask(task);
	assert.ok(size(excerpt) <= MODEL_ROUTING_TASK_BYTES);
...
test("oversized protected content is truncated with a marker instead of sent whole", () => {
	for (const close of ["", "</keepContext>"]) {
		const task = `start<keepContext>${"x".repeat(120_000)}${close}end`;
		const excerpt = modelRoutingTask(task);
		assert.ok(size(excerpt) <= MODEL_ROUTING_TASK_BYTES);
...
test("an oversized protected span before the objective keeps the objective in the routing excerpt", () => {
	const task = `<keepContext>${"constraint ".repeat(15_000)}</keepContext>\nImplement issue #3270.`;
	const excerpt = modelRoutingTask(task);
	assert.ok(size(excerpt) <= MODEL_ROUTING_TASK_BYTES);
